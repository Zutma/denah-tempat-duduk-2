...
function getPublishedSessions($conn) {
    $stmt = $conn->prepare("
...
function getPublishedSessionById($conn, $id) {
    $stmt = $conn->prepare("
        SELECT gs.*, ge.name AS event_name
        FROM graduation_sessions gs
        JOIN graduation_events ge ON gs.graduation_event_id = ge.id
...
function getSeatRowsWithSeats($conn, $sessionId, $side) {
...
function searchGraduates($conn, $sessionId, $keyword) {
    $like = '%' . $keyword . '%';
    $stmt = $conn->prepare("
