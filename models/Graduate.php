...
function getGraduatesBySession($conn, $sessionId, $limit = 20, $offset = 0) {
    $stmt = $conn->prepare("
...
function countGraduatesBySession($conn, $sessionId) {
...
function getAvailableSeats($conn, $sessionId) {
    $stmt = $conn->prepare("
...
function createGraduate($conn, $sessionId, $facultyId, $studyProgramId, $nrp, $name, $seatId) {
...
function deleteGraduate($conn, $id) {
    $stmt = $conn->prepare("DELETE FROM graduates WHERE id = ?");
    $stmt->bind_param("i", $id);
    return $stmt->execute();
}

function findOrCreateFaculty($conn, $code) {
...
function findOrCreateStudyProgram($conn, $facultyId, $name, $degreeLevel) {
...
function findSeatByPosition($conn, $sessionId, $row, $side, $position) {
    $stmt = $conn->prepare("
...
function seatIsTaken($conn, $seatId) {
