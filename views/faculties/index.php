...
<div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
    <div>
        ...
        <p class="text-xs text-slate-500 mt-1">Kelola data fakultas dan identitas warna penanda tempat duduk.</p>
    </div>
        ...
        <table class="w-full text-xs text-left text-slate-800 border-collapse">
            <thead class="bg-its-blue/5 border-b border-its-blue/10 text-its-blue uppercase font-bold text-[10px] tracking-wider">
                <tr>
                    ...
                    <th class="px-6 py-3.5 text-left">NAMA FAKULTAS</th>
                    <th class="px-6 py-3.5 text-left">WARNA PENANDA</th>
            ...
            <tbody class="divide-y divide-slate-100 bg-white">
                <?php if (empty($faculties)): ?>
                    <tr>
                        <td colspan="5" class="px-6 py-12 text-center text-xs text-slate-400 italic">Belum ada data fakultas.</td>
                    </tr>
                <?php else: ?>
                    <?php foreach ($faculties as $faculty): ?>
                        <tr class="hover:bg-slate-50/80 transition-colors">
                            <td class="px-4 py-4 text-center">
                                <input type="checkbox" class="rowCheckbox w-4 h-4 text-its-blue-light border-slate-300 rounded focus:ring-its-blue-light cursor-pointer" value="<?= $faculty['id'] ?>">
                            </td>
                            <td class="px-6 py-4">
                                <span class="inline-block px-2.5 py-1 text-xs font-bold font-mono bg-slate-100 text-slate-800 rounded-md border border-slate-200">
                                    <?= htmlspecialchars($faculty['code']) ?>
                                </span>
                            </td>
                            <td class="px-6 py-4 font-bold text-slate-800"><?= htmlspecialchars($faculty['name']) ?></td>
                            <td class="px-6 py-4">
                                <div class="inline-flex items-center gap-2 px-2.5 py-1 bg-slate-50 border border-slate-200 rounded-lg">
                                    <span class="inline-block w-3.5 h-3.5 rounded-full border border-slate-300 flex-shrink-0 shadow-2xs"
                            ...
                            <td class="px-6 py-4 text-right space-x-2 whitespace-nowrap">
                                ...
                                <a href="/faculties/delete?id=<?= $faculty['id'] ?>"
                                    onclick="return confirm('Yakin hapus fakultas ini?')"
