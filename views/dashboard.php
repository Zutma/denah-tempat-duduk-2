...
<div class="flex items-center justify-between gap-4 mb-8">
    ...
    <div class="flex items-center gap-4 px-4 py-2.5 bg-white rounded-xl border border-slate-200/80 shadow-2xs">
        ...
        <div class="text-sm text-slate-600 font-medium">
            <span class="font-bold text-slate-800 text-base"><?= $totalProdi ?></span> Program Studi
        </div>
    </div>
</div>
...
<div class="grid grid-cols-3 gap-6 mb-8">
    ...
    <div class="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-2xs flex items-center justify-between">
        <div>
            ...
            <h3 class="text-3xl font-extrabold text-slate-800 mt-1"><?= $draftSessionsCount ?? 0 ?></h3>
            <p class="text-xs text-slate-400 mt-1">Sesi belum dipublikasi</p>
        </div>
        <div class="w-12 h-12 rounded-xl bg-amber-50 text-amber-600 border border-amber-200/60 flex items-center justify-center shrink-0 text-xl">
            ✏️
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-2xs flex items-center justify-between">
        <div>
            ...
            <h3 class="text-3xl font-extrabold text-slate-800 mt-1"><?= $publishedSessionsCount ?? 0 ?></h3>
            <p class="text-xs text-slate-500 mt-1">
                <span class="font-bold text-slate-700"><?= number_format($activeGraduatesCount ?? 0) ?></span> Wisudawan Terplotting
            </p>
        </div>
        <div class="w-12 h-12 rounded-xl bg-emerald-50 text-emerald-600 border border-emerald-200/60 flex items-center justify-center shrink-0 text-xl">
            🌐
        </div>
    </div>
    ...
    <div class="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-2xs flex items-center justify-between">
        <div>
            ...
            <h3 class="text-3xl font-extrabold text-slate-800 mt-1"><?= $archivedSessionsCount ?? 0 ?></h3>
            <p class="text-xs text-slate-400 mt-1">Selesai dilaksanakan</p>
        </div>
        <div class="w-12 h-12 rounded-xl bg-slate-100 text-slate-500 border border-slate-200/60 flex items-center justify-center shrink-0 text-xl">
            📦
        </div>
    </div>
</div>
            ...
            <table class="w-full text-sm text-left text-slate-800 border-collapse">
                <thead class="bg-slate-50 text-slate-500 uppercase font-bold text-xs tracking-wider border-b border-slate-200/80">
                    <tr>
                        ...
                        <th class="px-6 py-4 text-center">JUMLAH SESI</th>
                        <th class="px-6 py-4 text-center">TOTAL WISUDAWAN</th>
                ...
                <tbody class="divide-y divide-slate-100 bg-white">
                    <?php if (empty($recentEvents)): ?>
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center text-sm text-slate-400 italic">Belum ada periode wisuda terdaftar.</td>
                        </tr>
                    <?php else: ?>
            ...
            <div class="space-y-3">
                <a href="/graduation-events/create" class="flex items-center justify-between p-4 rounded-xl bg-slate-50 hover:bg-its-blue/5 border border-slate-200/80 group transition-all">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-sm font-bold text-slate-800 group-hover:text-its-blue transition-colors">Buat Periode Wisuda</p>
                            <p class="text-xs text-slate-400">Tambah acara wisuda baru</p>
                        </div>
                    </div>
                ...
                <a href="/" target="_blank" class="flex items-center justify-between p-4 rounded-xl bg-its-blue text-white group transition-all hover:bg-its-blue-light">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-sm font-bold text-white">Lihat Denah Publik</p>
                            <p class="text-xs text-blue-100">Buka denah interaktif</p>
                        </div>
                    </div>
