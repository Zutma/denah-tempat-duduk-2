...
<div class="flex items-center justify-between gap-4 mb-6">
    <div>
        <h1 class="text-xl font-bold text-slate-800 tracking-tight"><?= !empty($event['id']) ? 'Edit Periode Wisuda' : 'Tambah Periode Wisuda' ?></h1>
        <p class="text-xs text-slate-500 mt-1">Lengkapi nama periode wisuda untuk mengelompokkan sesi alokasi kursi.</p>
    </div>
</div>
...
<div class="p-6 rounded-2xl shadow-2xs border border-slate-200/80 bg-white w-full">
    <form method="POST" class="space-y-5">
        ...
        <div>
            <label for="name" class="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1.5">Nama Periode Wisuda <span class="text-red-500">*</span></label>
        ...
        <div class="flex items-center justify-end gap-3 pt-4 border-t border-slate-100">
            ...
            <button type="submit"
                ...
                <?= !empty($event['id']) ? 'Update Periode' : 'Simpan Periode' ?>
            </button>
        </div>
    </form>
</div>
