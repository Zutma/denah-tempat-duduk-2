        ...
        <?php foreach ($events as $event): ?>
            <div class="bg-white rounded-2xl shadow-2xs border border-slate-200/80 p-6 hover:border-its-blue/30 transition-all flex flex-col justify-between">
                ...
                <div class="flex items-center justify-end gap-2 mt-6 pt-4 border-t border-slate-100">
                    ...
                    <a href="/graduation-events/delete?id=<?= $event['id'] ?>" onclick="return confirm('Yakin hapus?')" class="inline-flex items-center gap-1.5 px-3 py-2 text-xs font-bold text-red-600 bg-red-50 hover:bg-red-100 rounded-lg transition-colors border border-red-200/80">
                        Hapus
                    </a>
                </div>
            </div>
