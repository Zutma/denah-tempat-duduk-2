<h1 class="text-2xl font-bold text-slate-800 tracking-tight mb-6">
    <?= !empty($graduate) ? 'Edit Wisudawan' : 'Tambah Wisudawan' ?> — Sesi <?= !empty($session['date']) ? date('d F Y', strtotime($session['date'])) : '' ?>
</h1>
...
<div class="p-6 rounded-2xl shadow-2xs border border-slate-200/80 bg-white">
    <form method="POST" class="flex flex-col gap-4">
        <?= Csrf::field() ?>
        <?php if (!empty($graduate['id'])): ?>
            <input type="hidden" name="id" value="<?= htmlspecialchars($graduate['id']) ?>">
        <?php endif; ?>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" id="nrp" name="nrp" value="<?= htmlspecialchars($_POST['nrp'] ?? ($graduate['nrp'] ?? '')) ?>" required
            ...
            <div>
                ...
                <input type="text" id="name" name="name" value="<?= htmlspecialchars($_POST['name'] ?? ($graduate['name'] ?? '')) ?>" required
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                ...
                <?php $selectedFaculty = $_POST['faculty_id'] ?? ($graduate['faculty_id'] ?? ''); ?>
                <select id="faculty_id" name="faculty_id" required
                    ...
                    <?php foreach ($faculties as $faculty): ?>
                        <option value="<?= $faculty['id'] ?>" <?= (string)$selectedFaculty === (string)$faculty['id'] ? 'selected' : '' ?>>
            ...
            <div>
                ...
                <?php $selectedProdi = $_POST['study_program_id'] ?? ($graduate['study_program_id'] ?? ''); ?>
                <select id="study_program_id" name="study_program_id" required
                    class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-its-blue-sky/50 focus:border-its-blue-light bg-white cursor-pointer">
                    <option value="">-- Pilih Program Studi --</option>
                    <?php foreach ($studyPrograms as $sp): ?>
                        <option value="<?= $sp['id'] ?>" <?= (string)$selectedProdi === (string)$sp['id'] ? 'selected' : '' ?>>
                            <?= htmlspecialchars(($sp['degree_level'] ? $sp['degree_level'] . ' ' : '') . $sp['name']) ?>
                        </option>
                    <?php endforeach; ?>
                </select>
            </div>
        ...
        <div>
            ...
            <?php 
                $selectedSeatId = $_POST['seat_id'] ?? ($graduate['seat_id'] ?? '');
            ?>
            <select id="seat_id" name="seat_id"
                ...
                <?php else: ?>
                    <?php foreach ($seats as $s): ?>
                        <option value="<?= $s['id'] ?>" <?= ((string)$selectedSeatId === (string)$s['id']) ? 'selected' : '' ?>>
                            <?php if ($s['global'] !== null): ?>
                                Baris <?= htmlspecialchars($s['row']) ?> <?= $s['side'] == 'left' ? 'Kiri' : 'Kanan' ?> — Kursi <?= htmlspecialchars($s['row'] . sprintf('%03d', $s['global'])) ?>
                            <?php else: ?>
                                Baris <?= htmlspecialchars($s['row']) ?> <?= $s['side'] == 'left' ? 'Kiri' : 'Kanan' ?> — Posisi #<?= htmlspecialchars($s['local']) ?>
                            <?php endif; ?>
                        </option>
                    <?php endforeach; ?>
                <?php endif; ?>
