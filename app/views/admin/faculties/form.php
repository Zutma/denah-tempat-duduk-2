<h1 class="text-xl font-bold text-gray-800 mb-6"><?= !empty($faculty['id']) ? 'Edit Fakultas' : 'Tambah Fakultas' ?></h1>
...
<div class="p-6 rounded-xl shadow-sm border border-gray-200 bg-white">
    <form method="POST" class="flex flex-col gap-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
                <label for="code" class="block text-sm font-medium text-gray-700 mb-1">Kode Fakultas</label>
            ...
            <div>
                <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Nama Fakultas</label>
        ...
        <div x-data="{ hexColor: '<?= htmlspecialchars($faculty['color'] ?? '#127BBE') ?>' }">
            <label for="color" class="block text-sm font-medium text-gray-700 mb-1">Warna Penanda Kursi</label>
            <div class="flex items-center gap-3 max-w-xs">
                <input type="color" x-model="hexColor"
                    class="h-10 w-14 p-1 border border-gray-300 rounded-lg cursor-pointer bg-white">
